...
class Spot:
    def __init__(self, center: (float, float), radius: float, lvl):
        self._center = center
        self._radius = radius
        ...
        if lvl == 'Level Two':
            self._delta_x = (random.random() * 0.03) - 0.015  # change ball speed in level two.
            self._delta_y = (random.random() * 0.03) - 0.015
        else:
            self._delta_x = (random.random() * 0.01) - 0.005
            self._delta_y = (random.random() * 0.01) - 0.005
        self.sound = pygame.mixer.music.load('Beep_Short.mp3')
    ...
    def move(self) -> None:
        x, y = self._center
        if x <= 0:
            self._delta_x = -self._delta_x  # change x direction if the ball pass the boundary.
        if y <= 0:
            self._delta_y = -self._delta_y  # change y direction if the ball pass the boundary.
        if x >= 1:
            self._delta_x = -self._delta_x  # change x direction if the ball pass the boundary.
        if y >= 1:
            self._delta_y = -self._delta_y  # change y direction if the ball pass the boundary.
...
class SpotsState:
    ...
    def createten(self, point, lvl):
        if lvl == 'Level Two':
            self._spots.append(Spot(point, 0.045, lvl))  # change ball size.
        else:
            self._spots.append(Spot(point, 0.07, lvl))  # change ball size.
